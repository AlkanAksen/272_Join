...
class Schedule {
    ...
    void printTaskList(HashMap<Integer, Task> tasks) {
        ...
        for (Task t : tasks.values()) {
            ...
            if (t.taskState == stateT.undone) {
                ...
                String title = String.format(" %-25s", t.title);  // Format task title
    ...
    void printWishList(HashMap<Integer, Wish>  wishes) {
        ...
        for (Wish w : wishes.values()) {
            ...
            if (w.wishState != stateW.rejected) {
                ...
                String title = String.format(" %-25s", w.title);  // Format wish title
                // Print wish in table format with status (approved or on hold)
                System.out.println("│" + id + " │" + title + " │ " + (w.wishState == stateW.approved ? "Approved " : "On Hold  ") + "│");
            }
        }
        System.out.println("└────┴───────────────────────────┴──────────┘");  // End of table
    }
    ...
    void printScheduleDaily(HashMap<Integer, Task> tasks, HashMap<Integer, Wish> wishes) {
        ...
        for (Wish w : wishes.values()) {
            ...
            if (w.wishState == stateW.approved && w.hour1 != null && (currentDateTime.getDayOfYear() == w.date.getDayOfYear())) {
                String title = String.format(" %-25s", w.title);  // Format wish title
        ...
        for (Task t : tasks.values()) {
            ...
            if (t.taskState == stateT.undone && t.hour != null && (currentDateTime.getDayOfYear() == t.date.getDayOfYear())) {
                String title = String.format(" %-25s", t.title);  // Format task title
